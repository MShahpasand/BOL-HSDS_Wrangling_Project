...
def aggregate_to_person_FY(df_jj_admissions,financial_year_start,financial_year_end):
  ...
  df_jj_admissions['JJ_Adm'] = df_jj_admissions.AdmissionDate.dt.to_period('Q-JUN').dt.qyear.apply(lambda x : str(x-1)[2:]+str(x)[2:])
  df_jj_admissions = pd.get_dummies(df_jj_admissions,columns=['JJ_Adm'])
  df_jj_admissions = df_jj_admissions.reset_index()
